...
def build_ranked_candidate(**overrides) -> RankedCandidate:
    payload = {
...
def test_ranked_candidate_can_be_created_with_valid_data() -> None:
    candidate = build_ranked_candidate()

    assert isinstance(candidate.question, Question)
    assert candidate.final_score == 0.82
    assert isinstance(candidate.breakdown, SelectionBreakdown)
...
def test_ranked_candidate_final_score_cannot_be_negative() -> None:
    with pytest.raises(
        ValueError,
...
def test_ranked_candidate_rank_cannot_be_below_minimum() -> None:
    with pytest.raises(
        ValueError,
...
def test_ranked_candidate_rejects_invalid_field_types(
    field_name: str,
    value,
) -> None:
...
def test_ranked_candidate_rejects_bool_values_for_numeric_fields(
    field_name: str,
    value: bool,
) -> None:
    with pytest.raises(TypeError):
...
def test_ranked_candidate_rejects_non_finite_final_score(
    value: float,
) -> None:
    with pytest.raises(
        ValueError,
